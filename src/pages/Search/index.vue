      ...
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-for="(c, index) in searchParams.props" :key="index">
              {{ c.split(":")[1] }}
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      total: state => state.Home.getSearchList.total
    })
  },
  mounted() {
    this.getData();
  ...
  methods: {
    ...
    changeOrder(flag) {
      ...
      //获取到最开始的状态
      let startOrderFlag = this.searchParams.order.split(":")[0];
      let starSort = this.searchParams.order.split(":")[1];
      let newOrder = "";
      //点击的是价格
      if (flag == startOrderFlag) {
        newOrder = `${startOrderFlag}:${starSort == "desc" ? "asc" : "desc"}`;
      } else {
        newOrder = `${flag}:${"desc"}`;
      }
